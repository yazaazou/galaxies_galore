...
class GaussianDiffusion:
    ...
    def p_sample(self, pred_noise, x, t, clip_denoised=True):
        ...
        return model_mean + nonzero_mask * \
            tf.exp(0.5 * model_log_variance) * noise
...
def build_model(
    ...
    # MiddleBlock
    x = ResidualBlock(widths[-1], groups=norm_groups,
    ...
    x = activation_fn(x)
    x = layers.Conv2D(3, (3, 3), padding="same",
                      kernel_initializer=kernel_init(0.0))(x)
...
class DiffusionModel(keras.Model):
    ...
    def train_step(self, images):
        ...
        t = tf.random.uniform(
            minval=0, maxval=self.timesteps, shape=(
                batch_size,), dtype=tf.int32)

        with tf.GradientTape() as tape:
            # 3. Sample random noise to be added to the images in the batch
            noise = tf.random.normal(
                shape=tf.shape(images), dtype=images.dtype)
